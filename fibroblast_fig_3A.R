...
##################################################################################
######################## Main Figure DM at 4hrs, human ###########################
##################################################################################



p_dm <- calc_empirical_p(
  na.omit(gene_table$DM_pIC4[gene_table$divergence_group=="a_low_divergence"]),
  na.omit(gene_table$DM_pIC4[gene_table$divergence_group=="c_high_divergence"])
)
...
plots_gg = ggplot(gene_table[gene_table$divergence_group%in%c("a_low_divergence","b_middle_divergence","c_high_divergence"),], 
  ...
  coord_cartesian(ylim = c(-0.7, 1.85)) +
  ...
  geom_signif(comparisons=list(c("a_low_divergence","c_high_divergence")), annotations=formatSignificance(p_dm),
              y_position = 1.8, tip_length = 0, vjust=0.4) +
...
pdf("output_figures/fig_3A.pdf", width = 2.5, height = 3,useDingbats = F)
print(plots_gg)
dev.off()
